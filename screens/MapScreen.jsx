...
const MapScreen = () => {
    ...
    const coordinates = []
    const details = route.params.searchResults.map((item) => item.properties.map((property) => {
        coordinates.push({
        ...
        <View>
            <MapView ref={mapView} style={{ width: "100%", height: "100%" }} >
                {route.params.searchResults.map((item) => item.properties.map((property) => (
                    ...
                    <Marker title={property.name} coordinate={{ latitude: Number(property.latitude), longitude: Number(property.longitude) }} >
